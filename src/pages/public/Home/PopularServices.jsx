...
import  {homeContent}  from "../../../Config/content/index";
...
function PopularServices() {
    ...
    useEffect(() => {
        const fetchServices = async () => {
            try {
                setLoading(true);
                const data = await getPopularProfessionals(6);
                console.log("Popular services data:", data); // Debug log
                // Sort by popularity (you might need to adjust this based on your API response)
                // For now, we'll just take the first 10 services
                const popularServices = data.slice(0, 10);
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-30 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center rounded-[2rem] z-10">
                        ...
                        >
                            <button className="bg-red text-white py-2 px-6 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-semibold">
                                {t('home.popularServices.viewDetailsButton')}
                            </button>
                        </a>
                    </div>
            ...
            <div className="my-5">
                ...
                {services.length === 0 ? (
                    <div className="text-center py-10">
                        <p className="text-gray-500">{t('home.popularServices.emptyMessage')}</p>
                    </div>
                ) : (
                    <Swiper
